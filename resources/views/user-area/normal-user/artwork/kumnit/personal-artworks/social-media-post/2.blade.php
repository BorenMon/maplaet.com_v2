...
@section('css')
  <style>
    ...
    .artwork-preview .profile, .artwork-preview .quote-box {
      position: absolute;
    }
    ...
    <div class="customized-container">
      ...
      <div id="input-container">
        ...
        <div class="mb-3">
          ...
          <div class="flex">
            <input type="text" id="name" placeholder="ឈ្មោះម្ចាស់សម្ដី" style="width: calc(100% - 4rem) !important;"/>
            <button onclick="saveNameInput()" class="btn bg-gray-400 flex items-center justify-center" style="width: 3rem; margin-left: 1rem;"><i class="fa-solid fa-plus"></i></button>
          </div>
          <div class="flex flex-wrap mt-4 border saved-inputs" id="savedNameInputs">
            @foreach ($savedInputs as $input)
              @if ($input->category_name == 'Name')
...
<script>
  ...
  $('#name').on('input', function(){
    ...
    $('.name span').each((i, obj) => {
      obj.innerText = term
    })
  })
  const firstSavedName = $('#savedNameInputs div:first')
  const firstSavedNameContent = $('span', firstSavedName).text()
  firstSavedName.addClass('selected')
  $('#name').val(firstSavedNameContent)
  $('.name span').each(function() {
    this.innerText = firstSavedNameContent
  })
  const saveNameInput = () => {
    const dataSet = {
      category_name: 'Name',
      content: $('#name').val().trim(),
      _token: "{{ csrf_token() }}"
    }
    $.ajax({
      ...
      success: function(response) {
        ...
        `
        $('#savedNameInputs').append(content)

        $('span', content).on('click', function(){
          $('#savedNameInputs div').each(function(){
  ...
  $('#savedNameInputs div span').on('click', function(){
    $('#savedNameInputs div').each(function(){
      this.classList.remove('selected')
    })
    $(this).parent().addClass('selected')
  ...
  $('#savedNameInputs div i').on('click', function(){
    const content = $(this).parent()
    if(confirm('Are you sure to delete this saved input?')){
      let route = "{{ route('saved-input.destroy', ['saved_input' => ':id']) }}"
      route = route.replace(':id', $(this).data('id'))
