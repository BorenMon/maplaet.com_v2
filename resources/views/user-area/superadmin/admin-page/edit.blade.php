...
@section('header')
<i class="fa-solid fa-angle-left cursor-pointer" onclick="history.back()"></i>&emsp;Edit {{ $adminPage->name }}
@endsection
    ...
    <div id="form">
      <div id="inputs" class="space-y-4">
        ...
        <div class="label-input">
          ...
          <input type="text" id="name" class="mt-2" value="{{ $adminPage->name }}">
          <small class="text-maplaet-4 error" data-label="name"></small>
        </div>
        <div class="label-input">
          <label for="folder_name" class="font-semibold">Folder Name</label>
          <input type="text" id="folder_name" class="mt-2" value="{{ $adminPage->folder_name }}">
          <small class="text-maplaet-4 error" data-label="folder_name"></small>
        </div>
        <div class="label-input">
          <label for="is_active" class="font-semibold">Status</label>
          <select id="is_active">
            <option value="1" {{ $adminPage->is_active == '1' ? 'selected' : '' }}>Active</option>
            <option value="0" {{ $adminPage->is_active == '0' ? 'selected' : '' }}>Inactive</option>
          </select>
    ...
    if(file) {
      new Compressor(file, {
          ...
          success(result) {
            ...
            $.ajax({
              url: "{{ route('superadmin.admin-page.update', ['admin_page' => $adminPage->id]) }}",
              method: 'POST',
              data: formData,
              contentType: false,
              processData: false,
