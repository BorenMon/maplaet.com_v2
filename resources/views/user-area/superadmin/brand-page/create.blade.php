...
@section('header')
<i class="fa-solid fa-angle-left cursor-pointer" onclick="history.back()"></i>&emsp;Add New Brand Page To {{ $adminPage->name }}
@endsection
    ...
    if(file) {
      new Compressor(file, {
          ...
          success(result) {
            ...
            formData.append('admin_page_id', '{{ $adminPage->id }}')
            $.ajax({
              url: "{{ route('superadmin.brand-page.store') }}",
              type: 'POST',
              data: formData,
              contentType: false,
              processData: false,
